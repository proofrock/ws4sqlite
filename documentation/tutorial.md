...
{% hint style="success" %}
If ws4sql offers a relevant capability that doesn't fit in a tutorial, it will be explained in an info box like this, and a link to the relevant documentation will be provided.
{% endhint %}
...
This tells ws4sql to serve a database, to be created because a file at the specified path doesn't exist, using default settings. It's now possible to access the database using its id, that is the filename minus the suffix (in this case, `testDb`).
...
{% hint style="warning" %}
Please start from an empty database: stop ws4sql, remove the database file and start it again.
{% endhint %}
